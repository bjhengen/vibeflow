...
- **Pasting no longer stalls the window when another app is slow to hand over the clipboard** (#33). A paste is an X11 selection round-trip with whichever application owns the clipboard, and it ran on the UI thread — so a slow or wedged owner froze rendering and PTY draining for as long as it took, up to the 4 s that `arboard` allows before giving up. Clipboard *reads* now happen on a worker thread and the text is delivered back to the event loop, which stays responsive throughout. Clipboard writes are unchanged: they only store the data and assert selection ownership, so they do not block.

  Tabs now carry a stable id internally, because a read can outlive the tab's position in the bar: text is delivered to the tab the paste was requested from even if tabs were reordered meanwhile, and is dropped rather than redirected if that tab has closed.
